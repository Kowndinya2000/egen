...
import net.non_egen_osm_tracker.overlay.WayPointsOverlay;
...
public class DisplayTrackMap extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		x1 = findViewById(R.id.latitude);
		y1 = findViewById(R.id.longitude);
		x2 = findViewById(R.id.latitude2);
		y2 = findViewById(R.id.longitude2);
		total_distance = 0;
		currentTrackId = getIntent().getExtras().getLong(TrackContentProvider.Schema.COL_TRACK_ID);
		setTitle("(Non-eGEN)Track: " + currentTrackId + " Distance: 0m" );
		// Initialize OSM view
...
//	}
		...
		if (numberOfPointsRetrieved > 0 ) {
			...
			while(!c.isAfterLast()) {			
				...
				if(x1.getText().toString().isEmpty() || x2.getText().toString().isEmpty()){
					x1.setText(String.format("%s: %f", mLatitudeLabel,
							lastLat));
					y1.setText(String.format("%s: %f", mLongitudeLabel,
							lastLon));
				} else {
					initial = false;
					x1.setText(x2.getText().toString());
					y1.setText(y2.getText().toString());
				}
				x2.setText(String.format("%s: %f", mLatitudeLabel,
						lastLat));
				y2.setText(String.format("%s: %f", mLongitudeLabel,
						lastLon));
	...
	private void updateUI(double latitude2, double longitude2) {
		double latitude1 = Double.parseDouble(x1.getText().toString().substring(10));
		double longitude1 = Double.parseDouble(y1.getText().toString().substring(10));
		...
		total_distance = Double.valueOf(df.format(total_distance));
		setTitle("(Non eGEN)Track: " + currentTrackId + " Distance: " + Double.toString(total_distance) + "m" );
	}
}
